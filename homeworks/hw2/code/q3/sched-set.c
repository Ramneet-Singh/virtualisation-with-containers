...
main(int argc, char *argv[])
{
    ...
    // Acquire the CAP_SYS_NICE capability
    cap_t caps = cap_get_proc(); // Get current capabilities
    if (caps==NULL) errExit("cap_get_proc");

    // Add CAP_SYS_NICE to effective set
    ...
    cap_t empty;
    empty = cap_init();
    if (empty==NULL) errExit("cap_init");
    if (cap_set_proc(empty) == -1)
    {
      cap_free(empty);
      errExit("cap_set_proc");
    }
    if (cap_free(empty) == -1) errExit("cap_free");
